...
import com.centling.core.dao.IHrmWorkareaDao;
...
public class HrmEmployeeService implements IHrmEmployeeService {
	...
	public int listEmployeeCount(HrmEmployee employee,long l) {
		 System.out.println("页面传递过来的参数为"+employee.getHrmEmployeeSex());
	...
	public HrmEmployee saveEmployee(HrmEmployee employee) {
		//注意：插入数据时往数据库中插入的值就是1、2而不是男女
		System.out.println("获取的性别的值为："+employee.getHrmEmployeeSex());
