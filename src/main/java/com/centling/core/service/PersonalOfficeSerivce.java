...
import com.centling.common.pack.PersonalOfficeHqlPack;
...
import com.centling.core.dao.IHrmTimedrecordDao;
import com.centling.core.dao.IOaLeaveregistrationDao;
import com.centling.core.dao.IOaNotebookDao;
import com.centling.core.dao.IOaToolsDao;
import com.centling.core.dao.IOaTrsvelDao;
import com.centling.core.dao.ISysLibraryInfoDao;
import com.centling.core.iservice.IPersonalOfficeSerivce;
import com.centling.core.pojo.HrmTimedrecord;
import com.centling.core.pojo.OaLeaveregistration;
import com.centling.core.pojo.OaNotebook;
import com.centling.core.pojo.OaTools;
import com.centling.core.pojo.OaTrsvel;
...
public class PersonalOfficeSerivce implements IPersonalOfficeSerivce {
	...
	public void isHashToolsByEmpId(OaTools oaTools) {
		...
		if (count <= 0) {
			...
			List<SysLibraryInfo> list = librayInfoDao.findByHqlWhere(" and model.libraryInfoUpcode = '" + ConstWords.Tool_Libriary_Code + "' and model.companyId = " + oaTools.getCompanyId()
...
//				System.out.println(tmpcode);
	...
	public ProcessInstance startOaLeaveWorkflow(OaLeaveregistration oaLeaver,Map<String, Object> variables) {
	    ...
	    String businessKey = String.valueOf(oaLeaver.getPrimaryKey());
	    String key = EnumUtil.WORKFLOW_TYPE.LEAVE.key;

	    // 用来设置启动流程的人员ID，引擎会自动把用户ID保存到activiti:initiator中
	    identityService.setAuthenticatedUserId(oaLeaver.getApplyuser());

	    ProcessInstance processInstance = runtimeService.startProcessInstanceByKey(key, businessKey, variables);
	    String processInstanceId = processInstance.getId();
	    oaLeaver.setProcessInstanceId(processInstanceId);
	...
	public ProcessInstance startOaTrsvelWorkflow(OaTrsvel oaTrsvel,Map<String, Object> variables) {
	    ...
	    String businessKey = String.valueOf(oaTrsvel.getPrimaryKey());
	    String key = EnumUtil.WORKFLOW_TYPE.TRSVEL.key;
	    // 用来设置启动流程的人员ID，引擎会自动把用户ID保存到activiti:initiator中
	    identityService.setAuthenticatedUserId(oaTrsvel.getTrsvelApplyuser());

	    ProcessInstance processInstance = runtimeService.startProcessInstanceByKey(key, businessKey, variables);
	    String processInstanceId = processInstance.getId();
	    oaTrsvel.setProcessInstanceId(processInstanceId);
