...
function queryData(){
    ...
    var pager = getPager();
    dwrContractService.listIServiceInfo(contract,pager,queryCallback);
}
...
function dblCallback(obj){
    var box = new Sys.msgbox("明细查看","<%=contextPath%>/erp/memberui/contract_detail.jsp?pk="+obj.value);
    box.show();
}
function edit(pk){
    openMDITab("<%=contextPath%>/erp/memberui/contract_add.jsp?pk="+pk+"&tab="+getMDITab());
}
...
function del(pk){
    confirmmsg("确定要删除新建合同吗?","delok('"+pk+"')");
}
function delok(pk){
    var pks = new Array();
    pks[0] = pk;
    dwrContractService.deleteIContractByPks(pks,delCallback);
}
...
function delbatch(){
    if(getAllRecordArray() != false){
        confirmmsg("确定要删除新建合同吗?","delbatchok()");
    }else{
        alertmsg("请选择要删除的新建合同...");
    }
}
function delbatchok(){
...
function add(){
    openMDITab("<%=contextPath%>/erp/memberui/contract_add.jsp?tab="+getMDITab());
}
</script>
</head>
<body>
<%
SysGrid grid = new SysGrid(request,"新建合同列表");
//放入按钮
...
sccList.add(new SysColumnControl("contractCode","合同编码",1,1,1,0));
sccList.add(new SysColumnControl("contractName","合同名称",1,1,1,0));
sccList.add(new SysColumnControl("contractLimitType","期限类型",1,1,1,0));
sccList.add(new SysColumnControl("contractStatus","合同状态",1,1,1,0));
sccList.add(new SysColumnControl("contractTypeId","合同类型",1,1,1,0));
sccList.add(new SysColumnControl("contractBegindate","开始日期",1,1,1,0));
sccList.add(new SysColumnControl("contractEnddate","结束日期",1,1,1,0));
sccList.add(new SysColumnControl("contractContent","合同内容",1,1,1,0));
sccList.add(new SysColumnControl("contractFile","附件地址",1,1,1,0));
sccList.add(new SysColumnControl("memberId","会员ID",1,1,1,0));
sccList.add(new SysColumnControl("servicePackageId","套餐类型",1,1,1,0));
sccList.add(new SysColumnControl("contractPrice","合同金额",1,1,1,0));
sccList.add(new SysColumnControl("signedDate","签订日期",1,1,1,0));
