...
<title>服务套餐管理管理</title>
<script type="text/javascript" src="<%=contextPath%>/dwr/interface/dwrServicepackageService.js"></script>
<script type="text/javascript">
//查询方法
function queryData(){
    startQuery();
    var servicepackage = getQueryParam();//java实体类相对应
    var pager = getPager();
    dwrServicepackageService.listIServicePackage(servicepackage,pager,queryCallback);
}
...
function dblCallback(obj){
    var box = new Sys.msgbox("明细查看","<%=contextPath%>/erp/memberui/servicepackage_detail.jsp?pk="+obj.value);
    box.show();
}
function edit(pk){
    openMDITab("<%=contextPath%>/erp/memberui/servicepackage_add.jsp?pk="+pk+"&tab="+getMDITab());
}
...
function del(pk){
    confirmmsg("确定要删除服务套餐管理吗?","delok('"+pk+"')");
}
function delok(pk){
    var pks = new Array();
    pks[0] = pk;
    dwrServicepackageService.deleteIServicePackageByPks(pks,delCallback);
}
...
function delbatch(){
    if(getAllRecordArray() != false){
        confirmmsg("确定要删除服务套餐管理吗?","delbatchok()");
    }else{
        alertmsg("请选择要删除的服务套餐管理...");
    }
}
function delbatchok(){
...
function add(){
    openMDITab("<%=contextPath%>/erp/memberui/servicepackage_add.jsp?tab="+getMDITab());
}
</script>
</head>
<body>
<%
SysGrid grid = new SysGrid(request,"服务套餐管理列表");
//放入按钮
...
sccList.add(new SysColumnControl("servicePackageCode","编码",1,1,1,0));
sccList.add(new SysColumnControl("servicePackageName","名称",1,1,1,0));
sccList.add(new SysColumnControl("servicePackageAdviceprice","指导价格",1,1,1,0));
sccList.add(new SysColumnControl("servicePackagePrice","执行价格",1,1,1,0));
