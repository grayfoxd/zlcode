...
function queryData(){
    ...
    var pager = getPager();
    dwrContracttypeService.listIContractType(contracttype,pager,queryCallback);
}
...
function dblCallback(obj){
    var box = new Sys.msgbox("明细查看","<%=contextPath%>/erp/memberui/contracttype_detail.jsp?pk="+obj.value);
    box.show();
}
function edit(pk){
    openMDITab("<%=contextPath%>/erp/memberui/contracttype_add.jsp?pk="+pk+"&tab="+getMDITab());
}
...
function del(pk){
    confirmmsg("确定要删除会员分类管理吗?","delok('"+pk+"')");
}
function delok(pk){
    var pks = new Array();
    pks[0] = pk;
    dwrContracttypeService.deleteIContractTypeByPks(pks,delCallback);
}
...
function delbatch(){
    if(getAllRecordArray() != false){
        confirmmsg("确定要删除会员分类管理吗?","delbatchok()");
    }else{
        alertmsg("请选择要删除的会员分类管理...");
    }
}
function delbatchok(){
...
function add(){
    openMDITab("<%=contextPath%>/erp/memberui/contracttype_add.jsp?tab="+getMDITab());
}
</script>
</head>
<body>
<%
SysGrid grid = new SysGrid(request,"会员分类管理列表");
//放入按钮
...
sccList.add(new SysColumnControl("contractTypeCode","分类编码",1,1,1,0));
sccList.add(new SysColumnControl("contractTypeName","分类名称",1,1,1,0));
...
System.out.println(grid.createTable());
