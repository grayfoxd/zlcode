...
function queryData(){
    ...
    var pager = getPager();
    dwrService_infoService.listIServiceInfo(serviceinfo,pager,queryCallback);
}
...
function dblCallback(obj){
    var box = new Sys.msgbox("明细查看","<%=contextPath%>/erp/memberui/serviceinfo_detail.jsp?pk="+obj.value);
    box.show();
}
function edit(pk){
    openMDITab("<%=contextPath%>/erp/memberui/serviceinfo_add.jsp?pk="+pk+"&tab="+getMDITab());
}
...
function delok(pk){
    var pks = new Array();
    pks[0] = pk;
    dwrService_infoService.deleteIServiceInfoByPks(pks,delCallback);
}
...
function add(){
    openMDITab("<%=contextPath%>/erp/memberui/serviceinfo_add.jsp?tab="+getMDITab());
}
</script>
</head>
<body>
