					...
					<table width="99%" cellpadding="4" cellspacing="0" border="0"
						...
						<tr>
							<td style="width: 12%; border-bottom: 1px solid #BDBCBC;"
								...
								<label title="<%=levlMethod.getMethodMsg()%>" for="chk_<%=levlMethod.getPrimaryKey()%>"><%=levlMethod.getMethodInfoName()%></label>
							</td>
								...
								<table width="100%" cellpadding="4" cellspacing="1" border="0"
									...
									<tr>
										<td style="width: 12%" nowrap="nowrap">
											...
											<label title="<%=levlMethod2.getMethodMsg()%>" for="chk_<%=levlMethod2.getPrimaryKey()%>"><%=levlMethod2.getMethodInfoName()%></label>
										</td>
										<td>
											<table cellpadding="3" cellspacing="1" border="0"
												...
												<tr>
													...
													<td nowrap="nowrap">
														...
														<label title="<%=levlMethod3.getMethodMsg()%>" for="chk_<%=levlMethod3.getPrimaryKey()%>"><%=levlMethod3.getMethodInfoName()%></label>
													</td>
													<%
														td++;
																						if (td > 0 && td % 5 == 0) {
...
window.onload=function(){
	useLoadingMassage();
	initInput("helpTitle","权限设置，您可以在此处设置员工权限。");
...
function saverole(){
	...
	Btn.close();
	dwrSysProcessService.updateSysUserMethods('<%=userids%>',parray,methodcallback);
}
