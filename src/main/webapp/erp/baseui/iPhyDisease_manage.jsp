...
<title>疾病仓库维护管理</title>
<script type="text/javascript" src="<%=contextPath%>/dwr/interface/dwrDiseaseService.js"></script>
<script type="text/javascript">
//查询方法
function queryData(){
    startQuery();
    var iphydisease = getQueryParam();//java实体类相对应
    var pager = getPager();
    dwrDiseaseService.listIPhyDisease(iphydisease,pager,queryCallback);
}
...
function dblCallback(obj){
    var box = new Sys.msgbox("明细查看","<%=contextPath%>/erp/baseui/iPhyDisease_detail.jsp?pk="+obj.value);
    box.show();
}
function edit(pk){
    openMDITab("<%=contextPath%>/erp/baseui/iPhyDisease_add.jsp?pk="+pk+"&tab="+getMDITab());
}
...
function del(pk){
    confirmmsg("确定要删除疾病仓库维护吗?","delok('"+pk+"')");
}
function delok(pk){
    var pks = new Array();
    pks[0] = pk;
    dwrDiseaseService.deleteIPhyDiseaseByPks(pks,delCallback);
}
...
function delbatch(){
    if(getAllRecordArray() != false){
        confirmmsg("确定要删除疾病仓库维护吗?","delbatchok()");
    }else{
        alertmsg("请选择要删除的疾病仓库维护...");
    }
}
function delbatchok(){
...
function add(){
    openMDITab("<%=contextPath%>/erp/baseui/iPhyDisease_add.jsp?tab="+getMDITab());
}
</script>
</head>
<body>
<%
SysGrid grid = new SysGrid(request,"疾病仓库维护列表");
//放入按钮
...
sccList.add(new SysColumnControl("diseaseName","诊断名称",1,1,1,10));
sccList.add(new SysColumnControl("diseaseComment","解说",1,1,1,20));
sccList.add(new SysColumnControl("diseaseClinical","临床意义",1,1,1,30));
sccList.add(new SysColumnControl("diseaseAdvise","建议",1,1,1,20));
sccList.add(new SysColumnControl("conditions","条件",2,1,2,0));
sccList.add(new SysColumnControl("conditionsDetail","条件细节",2,1,2,0));
...
grid.setTableRowSize(30);//每页显示多少条数据
