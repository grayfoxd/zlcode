...
function queryData(){
    startQuery();
    var iPhyPackage = getQueryParam();//java实体类相对应
    var pager = getPager();
    dwrPhyPackageService.listIPhyPackage(iPhyPackage,pager,queryCallback);
}
...
function dblCallback(obj){
    var box = new Sys.msgbox("明细查看","<%=contextPath%>/erp/baseui/i_phy_package_detail.jsp?pk="+obj.value);
    box.show();
}
function edit(pk){
    openMDITab("<%=contextPath%>/erp/baseui/i_phy_package_edit.jsp?pk="+pk+"&tab="+getMDITab());
}
...
function del(pk){
    confirmmsg("确定要删除体检套餐吗?","delok('"+pk+"')");
}
function delok(pk){
    var pks = new Array();
    pks[0] = pk;
    dwrPhyPackageService.deleteIPhyPackageByPks(pks,delCallback);
}
...
function delbatch(){
    if(getAllRecordArray() != false){
        confirmmsg("确定要删除体检套餐吗?","delbatchok()");
    }else{
        alertmsg("请选择要删除的体检套餐...");
    }
}
function delbatchok(){
...
function add(){
    openMDITab("<%=contextPath%>/erp/baseui/i_phy_package_add.jsp?tab="+getMDITab());
}
</script>
</head>
<body>
<%
SysGrid grid = new SysGrid(request,"体检套餐列表");
//放入按钮
...
sccList.add(new SysColumnControl("packageName","套餐名称",1,1,1,0));
sccList.add(new SysColumnControl("marketPrice","市场价格",1,1,1,0));
sccList.add(new SysColumnControl("actualPrice","实际价格",1,1,1,0));
sccList.add(new SysColumnControl("gender","适用性别",1,1,1,0));
sccList.add(new SysColumnControl("compnayId","公司ID",1,1,1,0));
