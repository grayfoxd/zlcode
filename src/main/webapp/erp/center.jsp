...
function loadxml_apply(type,objid){
	...
	jQuery.get("<%=contextPath%>/erp/desktop/desktopHtml_apply.jsp?applyType="+type,
    function(str){
		<%if(applyshow){%>
...
function showProcessTrace(instanceId){
	var box = new Sys.msgbox('查看办理进度','<%=contextPath %>/processTrace.do?id='+instanceId,750,500);
	box.show();
}
		...
		<div class="dragLayer" id="applywin">
			<div class="dragHeader">
				<div class="dragleft" style="background: url('<%=contextPath %>/images/projectimg/apply.png') 3px no-repeat;">
					...
					&nbsp;<span class="more" title="更多" onclick="moreApplyByType()">
					</span>
				</div>
			</div>
			<div class="content">
